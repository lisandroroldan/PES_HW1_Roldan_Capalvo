function [Ke,fe] = MatEl(Xe,nnode,pospg,wpg,N,dNdxi,ncoord,nu_el) 
...
for igaus = 1:ngaus 
    ...
    N_igaus = N(igaus,:);  %%Recovered original expresion instead of  N(:,igaus)'
    ...
    Nx = res(1,:) ;
    Ny = res(2,:) ;        
    Ke = Ke + (nu_el*(Nx'*Nx+Ny'*Ny))*dvolu;
    elseif ncoord==3
    Nxi_igaus = dNdxi(3*igaus-2,:);
    Neta_igaus = dNdxi(3*igaus-1,:);
    Nmu_igaus = dNdxi(3*igaus,:);
    ...
    Ny = res(2,:) ;
    Nz = res(3,:) ;
    Ke = Ke + (nu_el*(Nx'*Nx+Ny'*Ny+Nz'*Nz))*dvolu;
